...
public class MovieJsonUtilities {
    public static String getMovieId(Context context, String movieIdJson) throws JSONException{
    ...
    public static ArrayList<String> getMovieDetailsFromJson(Context context, String movieDetailsJson) throws JSONException{
        ...
        JSONObject obj = new JSONObject(movieDetailsJson);

        list.add(obj.getString(OWM_TITLE));

        list.add(obj.getString(OWM_RELEASE_DATE));

        list.add(obj.getString(OWM_OVERVIEW));
    ...
    public static ArrayList<String> getMovieImagesFromJson(Context context, String movieImagesJson) throws JSONException {
        ArrayList<String> list = new ArrayList<String>();
            final String OWM_FILEPATH = "file_path";

            JSONObject obj = new JSONObject(movieImagesJson);
            JSONArray array = obj.getJSONArray("posters");
            for (int i = 0; i < array.length()/3; i++) {
                JSONObject images = array.getJSONObject(i);
                list.add(images.getString(OWM_FILEPATH));
            }
            return list;
